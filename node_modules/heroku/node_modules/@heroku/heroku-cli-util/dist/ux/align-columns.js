 ...
 *   ]
 */
export function alignColumns(arrays) {
    // eslint-disable-next-line no-control-regex
    const ansiRegex = /\u001B\[\d+m/g;
    const visualLength = (str) => str.replaceAll(ansiRegex, '').length;
    if (arrays.length === 0) {
        return [];
    }
    // calculate width for each column based on max visual length of entries
    const columnWidths = arrays[0].map((_, colIndex) => Math.max(...arrays.map(row => visualLength(row[colIndex]))));
    // align rows by padding to max length
    const alignedRows = arrays.map(row => row.map((cell, colIndex) => {
