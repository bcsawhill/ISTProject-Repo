 ...
 */
export async function promptAndRun(options) {
    ...
    for (const [argKey, argDef] of Object.entries(commandMeta.args)) {
        const { description, required } = argDef;
        if (!description)
            continue;
    ...
    for (const [, flagDef] of Object.entries(commandMeta.flags)) {
        const { char, description, hidden, name, options, type } = flagDef;
        // Skip hidden flags, the prompt flag itself, and flags without descriptions
        if (!description || hidden || name === 'prompt')
            continue;
