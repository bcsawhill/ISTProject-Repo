...
export default class ConfirmCommand {
    async confirm(app, confirm, message) {
        if (confirm) {
            if (confirm === app)
                return;
...
This command will affect the app ${color.app(app)}`;
        ...
        ux.warn(message);
        console.error();
        const entered = await hux.prompt(`To proceed, type ${color.warning(app)} or re-run this command with ${color.code('--confirm ' + app)}`, { required: true });
        if (entered === app) {
            return;
        }
