 ...
 */
function getComplianceMessage(quota) {
    const currentUsage = quota.current_gb ?? 0;
    if (!quota.critical_gb && !quota.warning_gb)
        return '(No quotas set)';
    if (quota.critical_gb && currentUsage > quota.critical_gb && quota.enforcement_active)
        return '(Restricted)';
    if (quota.critical_gb && currentUsage > quota.critical_gb)
        return '(Exceeded configured critical quota)';
    if (quota.warning_gb && currentUsage > quota.warning_gb)
        return '(Exceeded configured warning quota)';
 ...
 */
export const formatQuotaStatus = (quota) => {
...
export const displayQuota = (quota) => {
    ...
    }, ['Warning', 'Critical', 'Enforcement Action', 'Status']);
};
