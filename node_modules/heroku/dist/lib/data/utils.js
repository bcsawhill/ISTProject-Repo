...
import { MaintenanceStatus, } from './types.js';
 ...
 */
export function renderPricingInfo(pricingInfo, count = 1) {
    if (!pricingInfo)
        return '';
 ...
 */
const levelsAndPricingCache = new Map();
/**
 ...
 * @returns void
 */
export function clearLevelsAndPricingCache() {
 ...
 */
export async function fetchLevelsAndPricing(tier, dataApi) {
    const cacheKey = `levels-pricing-${tier}`;
    if (!levelsAndPricingCache.has(cacheKey)) {
        const promise = (async () => {
            const levelsPromise = dataApi.get('/data/postgres/v1/levels/advanced');
            const pricingPromise = dataApi.get('/data/postgres/v1/pricing');
 ...
 */
export async function renderLevelChoices(extendedLevelsInfo, pool, withGoBack = false) {
    const choices = [];
    extendedLevelsInfo.forEach(level => {
        const columns = [];
        columns.push(`${level.name}`, `${printf('%3d', level.vcpu)} ${color.inverse('vCPU')} `, `${printf('%4d', level.memory_in_gb)} GB ${color.inverse('MEM')} `, `starting at ${color.green(renderPricingInfo(level.pricing))}`);
...
const retryAsync = async (retries, delay, fn, current = 0) => {
    if (current >= retries) {
...
export async function waitUntilMaintenanceComplete(addonId, shogun) {
    return retryAsync(Number.POSITIVE_INFINITY, 5000, async () => {
        const { body: maintenance } = await shogun.get(`/data/maintenances/v1/${addonId}`, shogun.defaults);
