...
const lineRegex = /^(.*?\[([\w-]+)([\d.]+)?]:)(.*)?$/;
const dim = (i) => color.dim(i);
const other = dim;
const path = (i) => color.green(i);
const method = (i) => color.magenta(i);
const status = (code) => {
    if (code < 200)
        return code;
    if (code < 300)
        return color.success(code);
    if (code < 400)
        return color.info(code);
    if (code < 500)
        return color.warning(code);
    if (code < 600)
        return color.failure(code);
    return code;
};
const ms = (s) => {
...
export default function colorize(line) {
...
function colorizeAPI(body) {
    if (body.match(/^Build succeeded$/))
        return color.success(body);
    if (body.match(/^Build failed/))
        return color.failure(body);
