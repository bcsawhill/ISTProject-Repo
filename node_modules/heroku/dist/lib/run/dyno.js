...
export default class Dyno extends Duplex {
    ...
    _env() {
        const c = this.opts.env ? buildEnvFromFlag(this.opts.env) : {};
    ...
    _readData(c) {
        ...
        return (data) => {
            ...
            if (c && firstLine) {
                if (this.opts.showStatus)
                    ux.action.stop(this._status('up'));
                firstLine = false;
                this._readStdin(c);
                return;
            }
            ...
            if (exitCode) {
                ...
                if (code === 0) {
                    ...
                    const err = new Error(`Process exited with code ${color.failure(code.toString())}`);
                    err.exitCode = code;
                    // @ts-ignore
                    this.reject(err);
                }
