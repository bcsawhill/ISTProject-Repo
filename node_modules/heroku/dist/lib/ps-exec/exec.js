...
export class HerokuExec {
    async checkStatus(context, heroku, configVars) {
        const { body: dynos } = await heroku.request(`/apps/${context.app}/dynos`);
    ...
    createSocksProxy(context, heroku, configVars, callback) {
        return this.updateClientKey(context, heroku, configVars, (privateKey, dyno, response) => {
            ...
            new HerokuSsh().socksv5(json.tunnel_host, json.client_user, privateKey, json.proxy_public_key, socks_port => {
                if (callback)
                    callback(json.dyno_ip, dyno, socks_port);
                else
                    ux.stdout(`Use ${color.command('CTRL+C')} to stop the proxy`);
            });
        });
    }
    async initFeature(context, heroku, callback, command) {
        const buildpackUrls = ['https://github.com/heroku/exec-buildpack', 'urn:buildpack:heroku/exec'];
        const { body: app } = await heroku.get(`/apps/${context.app}`, {
            headers: {
        ...
        const [{ body: buildpacks }, { body: configVars }, { body: feature }] = await Promise.all([
            heroku.get(`/apps/${context.app}/buildpack-installations`),
            heroku.get(`/apps/${context.app}/config-vars`),
            heroku.get(`/apps/${context.app}/features/runtime-heroku-exec`),
        ]);
        ...
        if (addonUrl) {
            ...
            while (state !== 'up') {
                ...
                const { body: dyno } = await heroku.request(`/apps/${context.app}/dynos/${dynoName}`);
                state = dyno.state;
                if (state === 'crashed') {
                    ux.action.stop();
    ...
    async updateClientKey(context, heroku, configVars, callback) {
    ...
    async _enableFeature(context, heroku) {
    ...
    _hasExecBuildpack(buildpacks, urls) {
        ...
        for (const b of buildpacks) {
            for (const u of urls) {
                if (b.buildpack.url && b.buildpack.url.indexOf(u) === 0)
                    return true;
            }
        }
        return false;
