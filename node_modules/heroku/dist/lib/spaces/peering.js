...
export function displayPeeringInfo(space, info) {
...
export function displayPeeringsAsJSON(peerings) {
    ux.stdout(JSON.stringify(peerings, null, 2));
}
export function displayPeerings(space, peerings) {
    ...
    hux.table(peerings, {
        ...
        cidr_blocks: {
            header: 'CIDR Blocks',
            get: (row) => displayCIDR(row.cidr_blocks),
        },
        status: {
            header: 'Status',
            get: (row) => peeringStatus(row.status),
        },
        aws_vpc_id: {
            header: 'VPC ID',
        },
