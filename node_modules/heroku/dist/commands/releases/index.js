    ...
    for (const row of releases) {
        for (const colKey in row) {
            ...
            for (const [key, col] of Object.entries(columns)) {
                ...
                if (key !== optimizeKey) {
                    optimizationWidthMap[key] = Math.max(optimizationWidthMap[key], ansis.strip(String(formattedValue)).length);
                }
            }
        }
    }
...
export default class Index extends Command {
    ...
    static examples = [
        `${color.command('v1 Config add FOO_BAR email@example.com 2015/11/17 17:37:41 (~ 1h ago)')}`,
    ...
    async run() {
        ...
        const getVersionColor = (release) => {
            const statusColor = statusHelper.color(release.status);
            if (statusColor === 'red')
                return color.failure('v' + release.version);
            if (statusColor === 'yellow')
                return color.warning('v' + release.version);
            if (statusColor === 'gray')
                return color.inactive('v' + release.version);
