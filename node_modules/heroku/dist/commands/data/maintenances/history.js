...
export default class DataMaintenancesHistory extends BaseCommand {
    ...
    async run() {
        ...
        const { body: { maintenances } } = await this.dataApi.get(`/data/maintenances/v1/${addon.id}/history/?${queryParams.toString()}`, this.dataApi.defaults);
        ux.action.stop();
        ...
        const allTableColumns = {
            scheduled_for: {
                get: (row) => row && row.scheduled_for ? row.scheduled_for : '-',
                header: 'Scheduled for',
            },
            started_at: {
                get: (row) => row && row.started_at ? row.started_at : '-',
                header: 'Started at',
            },
            completed_at: {
                get: (row) => row && row.completed_at ? row.completed_at : '-',
                header: 'Completed at',
            },
            duration_seconds: {
                get: (row) => row && row.duration_seconds ? row.duration_seconds : '-',
            ...
            window: {
                get: (row) => row && row.window ? row.window : '-',
                header: 'Window',
            },
        };
