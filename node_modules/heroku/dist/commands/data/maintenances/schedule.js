...
export default class DataMaintenancesSchedule extends BaseCommand {
    ...
    async computeDelayWeeks(addon, week) {
        const { body: maintenance } = await this.dataApi.get(`/data/maintenances/v1/${addon.id}`, this.dataApi.defaults);
        const scheduled = (maintenance.status === 'completed' || maintenance.scheduled_for === null)
            ? Date.now()
            : Date.parse(maintenance.scheduled_for);
        const weeks = differenceInCalendarWeeks(Date.parse(week), scheduled);
    ...
    async scheduleMaintenance(addon, delayWeeks) {
        ux.action.start(`Scheduling maintenance for ${color.addon(addon.name)}`);
        const { body: schedule } = await this.dataApi.post(`/data/maintenances/v1/${addon.id}/schedule`, {
