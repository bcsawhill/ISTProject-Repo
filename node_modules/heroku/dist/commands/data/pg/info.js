...
function renderPoolSummary(pool, attachments) {
...
export default class DataPgInfo extends BaseCommand {
    ...
    async run() {
        ...
        const [{ body: info }, { body: attachments }] = await Promise.all([
            this.dataApi.get(`/data/postgres/v1/${addon.id}/info`),
            this.heroku.get(`/addons/${addon.id}/addon-attachments`),
        ]);
        const { created_at: createdAt, features, forked_from: forkedFrom, pools, quotas, region, status, tier, version, } = info;
