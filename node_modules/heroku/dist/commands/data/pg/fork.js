...
export default class Fork extends BaseCommand {
    ...
    async notify(message, success = true) {
     ...
     */
    parseRollbackInterval(interval) {
    ...
    async run() {
        ...
        const renderLegacyCommand = () => `heroku addons:create ${addon.plan.name}`
            + ` -a ${app}`
            + `${name ? ` --name ${name}` : ''}`
            + `${as ? ` --as ${as}` : ''}`
            + `${wait ? ' --wait' : ''}`
        ...
        try {
            ...
            ux.action.stop();
            if (wait) {
                this.notify('We can\'t provision the database fork. Try again or open a ticket with Heroku Support: https://help.heroku.com/.', false);
            }
            throw error;
        }
    }
