...
export default class DataPgUpdate extends BaseCommand {
    ...
    database;
    extendedLevelsInfo;
    followerInstanceCount = 0;
    pool;
    selectedPoolOption;
    async confirmCommand(comparison) {
        await hux.confirmCommand({ comparison });
    }
    async followerPoolActionStep(pool) {
        const choices = [
    ...
    async leaderPoolActionStep(pool) {
        ...
        if (pool.expected_count > 1) {
            ...
            choices.push({
                name: ('Add a high availability (HA) standby instance'
    ...
    async run() {
        ...
        if (dbIdentifier) {
          ...
          Use ${color.code(`heroku addons:upgrade ${this.database.name} -a ${app}`)} instead.`);
            ...
            const selectedDatabase = (await this.prompt({
     ...
     */
    async allAdvancedDatabaseAttachments(app) {
     ...
     */
    async getAllAdvancedDatabases(app) {
    ...
    async poolSelectionStep() {
        // Fetch the info each time we render this menu to show the updated database configuration
        const { body: databaseInfo } = await this.dataApi.get(`/data/postgres/v1/${this.database.id}/info`);
        ...
        const selectedPoolOption = (await this.prompt({
    ...
    renderDatabaseChoices(databases) {
        const choices = [];
    ...
    async renderPoolChoices(pools) {
        ...
        choices.push(new Separator(), { name: 'Add a follower pool', value: '__add_follower_pool' }, { name: 'Exit', value: '__exit' });
        return choices;
    }
}
