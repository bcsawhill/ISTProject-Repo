...
const settingsChangeTableData = (response) => response.changes.map(change => ({
    From: color.yellow(change.previous),
    Settings: change.name,
    To: color.cyan(change.current),
}));
const settingsTableData = (response) => {
...
export default class DataPgSettings extends BaseCommand {
    ...
    async run() {
        ...
        const { app, set } = flags;
        const settings = set?.map((setting) => setting.trim()).join(',');
      ...
      `));
        ...
        if (settings) {
            const response = await this.dataApi.put(`/data/postgres/v1/${database.id}/settings`, {
                body: { settings },
            });
            const { body } = response;
            ...
            const response = await this.dataApi.get(`/data/postgres/v1/${database.id}/settings`);
            const { body } = response;
