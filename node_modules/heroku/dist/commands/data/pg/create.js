...
export default class DataPgCreate extends BaseCommand {
    ...
    async run() {
        ...
        try {
            ...
            ux.action.stop();
            if (wait) {
                notify('heroku data:pg:create', 'We can’t provision the database. Try again or open a ticket with Heroku Support: https://help.heroku.com/.', false);
            }
            throw error;
        }
    ...
    async leaderConfirmationStep() {
      ...
      ${`${color.green('✓ Configure Leader Pool')} ${totalPrice}`}
        ${color.dim(`${this.leaderLevel} ${leaderLevelInfo?.vcpu} ${color.inverse('vCPU')} `
