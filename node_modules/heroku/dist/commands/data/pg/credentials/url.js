...
export default class Url extends BaseCommand {
    ...
    async run() {
        ...
        let credInfo;
        if (isAdvancedTier) {
            const { body: { items: availableCreds } } = await this.dataApi.get(`/data/postgres/v1/${addon.id}/credentials`);
            ...
            ({ body: credInfo } = await this.dataApi.get(`/data/postgres/v1/${addon.id}/credentials/${encodeURIComponent(credName)}`));
        }
        else {
            credName = name ?? 'default';
            ({ body: credInfo } = await this.dataApi.get(`/postgres/v0/databases/${addon.id}/credentials/${encodeURIComponent(credName)}`));
        }
