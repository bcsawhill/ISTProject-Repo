...
export default class Rotate extends BaseCommand {
    ...
    getCredAttachmentsAndUniqueAppNames(isAdvancedTier, isEssentialTier, credToRotate, attachments) {
        let uniqueAppNames = '';
    ...
    async getCredsAndAttachments(isAdvancedTier, addon) {
        let availableCreds;
        let attachments;
        if (isAdvancedTier) {
            [{ body: { items: availableCreds } }, { body: attachments }] = await Promise.all([
                this.dataApi.get(`/data/postgres/v1/${addon.id}/credentials`),
                this.heroku.get(`/addons/${addon.id}/addon-attachments`),
            ]);
        }
        else {
            [{ body: availableCreds }, { body: attachments }] = await Promise.all([
                this.dataApi.get(`/postgres/v0/databases/${addon.id}/credentials`),
                this.heroku.get(`/addons/${addon.id}/addon-attachments`),
            ]);
        }
        return { attachments, availableCreds };
    }
    getCredToRotate(isAdvancedTier, availableCreds, isEssentialTier, credName) {
        let credToRotate;
        let ownerCred;
        let defaultCred;
        if (isAdvancedTier) {
            ownerCred = availableCreds.find((cred) => cred.type === 'owner');
            credToRotate = credName ? availableCreds.find((cred) => cred.name === credName) : ownerCred;
        }
        else {
            defaultCred = isEssentialTier ? availableCreds[0] : availableCreds.find((cred) => cred.name === 'default');
            credToRotate = credName && !isEssentialTier ? availableCreds.find((cred) => cred.name === credName) : defaultCred;
        }
        return { credToRotate, defaultCred, ownerCred };
    }
    renderWarnings({ all, credToRotate, defaultCred, force, isAdvancedTier, isEssentialTier, ownerCred, }) {
        const warnings = [];
