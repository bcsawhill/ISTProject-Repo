...
export default class DataPgQuotasUpdate extends BaseCommand {
    ...
    async run() {
        ...
        const quotaUpdate = {};
        if (warning)
            quotaUpdate.warning_gb = warning === 'none' ? null : Number.parseInt(warning, 10);
        if (critical)
            quotaUpdate.critical_gb = critical === 'none' ? null : Number.parseInt(critical, 10);
        if (enforcementAction)
            quotaUpdate.enforcement_action = enforcementAction;
        ux.action.start(`Updating ${type} quota on ${color.datastore(database)}`);
        const { body: updatedQuota } = await this.dataApi.patch(`/data/postgres/v1/${addon.id}/quotas/${type}`, {
            body: quotaUpdate,
        });
        ux.action.stop();
