...
export default class DataPgQuotasIndex extends BaseCommand {
    ...
    async run() {
        ...
        if (type) {
            const { body: quota } = await this.dataApi.get(`/data/postgres/v1/${addon.id}/quotas/${type}`);
            displayQuota(quota);
        }
        else {
            const { body: quotas } = await this.dataApi.get(`/data/postgres/v1/${addon.id}/quotas`);
