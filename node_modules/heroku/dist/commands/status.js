...
const getTrustStatus = async () => {
    ...
    try {
        const [instanceResponse, activeIncidentsResponse, maintenancesResponse, localizationsResponse] = await Promise.all([
            HTTP.get(`${trustHost}/instances?products=Heroku`),
            HTTP.get(`${trustHost}/incidents/active`),
            HTTP.get(`${trustHost}/maintenances?startTime=${currentDateTime}&limit=10&offset=0&product=Heroku&locale=en`),
            HTTP.get(`${trustHost}/localizations?locale=en`),
        ]);
    ...
    if (incidents.length > 0) {
        incidents.forEach(incident => {
            incident.IncidentEvents.forEach(event => {
                event.localizedType = localizations.find((l) => l.modelKey === event.type)?.text;
            });
        });
    }
    ...
    async run() {
        ...
        if (process.env.TRUST_ONLY) {
            ...
            try {
                ...
                const herokuStatusResponse = await HTTP.get(herokuHost + herokuApiPath);
