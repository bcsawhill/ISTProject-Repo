...
export default class Upgrade extends Command {
  ...
  `);
    ...
    async run() {
        ...
        const { body: replica } = await this.heroku.get(`/client/v11/databases/${db.id}`, { hostname: utils.pg.host() });
      ...
      Destructive action
      You're canceling the scheduled version upgrade for ${color.datastore(db.name)}.

      You can't undo this action.
    `));
        try {
            ux.action.start(`Cancelling upgrade on ${color.addon(db.name)}`);
            const response = await this.heroku.post(`/client/v11/databases/${db.id}/upgrade/cancel`, { body: {}, hostname: utils.pg.host() });
