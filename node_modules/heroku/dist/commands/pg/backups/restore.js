...
export default class Restore extends Command {
    ...
    async run() {
        ...
        if (backupName && backupName.match(/^https?:\/\//)) {
            ...
            const { body: transfers } = await this.heroku.get(`/client/v11/apps/${backupApp}/transfers`, { hostname: utils.pg.host() });
    ...
    `));
        const { body: restore } = await this.heroku.post(`/client/v11/databases/${db.id}/restores`, {
            body: { backup_url: backupURL, extensions: this.getSortedExtensions(extensions) }, hostname: utils.pg.host(),
        });
        ux.action.stop();
        await pgbackups.wait('Restoring', restore.uuid, interval, verbose, db.app.id);
    }
}
