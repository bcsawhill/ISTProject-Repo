...
const node_repl_fix_completer_side_effects_1 = require("./node-repl-fix-completer-side-effects");
const node_repl_fix_history_rewrite_on_error_1 = require("./node-repl-fix-history-rewrite-on-error");
class MongoshNodeRepl {
    ...
    async initialize(serviceProvider, greeting) {
        ...
        if (!usePlainVMContext) {
            ...
            (0, repl_paste_support_1.fixNode60446)(repl);
            context = repl.context;
            repl[util_2.default.inspect.custom] = () => {
        ...
        context.Date = Date;
        {
            const globalCryptoDescriptor = Object.getOwnPropertyDescriptor(context, 'crypto') ?? {};
            if (globalCryptoDescriptor.value?.subtle ||
    ...
    async finishInitializingNodeRepl() {
        ...
        const origReplCompleter = await (0, node_repl_fix_completer_side_effects_1.fixNodeReplCompleterSideEffectHandling)((0, util_1.promisify)(repl.completer.bind(repl)));
        await (0, node_repl_fix_history_rewrite_on_error_1.fixNodeReplHistoryHandler)(repl);
        ...
        if (process.env.USE_NEW_AUTOCOMPLETE !== '0') {
            ...
            oldMongoshCompleter = autocomplete_1.completer.bind(null, autocompleteParams);
        }
                ...
                (async () => {
                    if (process.env.USE_NEW_AUTOCOMPLETE !== '0') {
                        if (!newMongoshCompleter) {
                            newMongoshCompleter = await (0, autocomplete_1.initNewAutocompleter)(instanceState);
                        }
        ...
        asyncRepl.addReplEventForEvalReady(repl, () => !!this.started, () => this.lineByLineInput.nextLine());
    ...
    async onAsyncSigint() {
        ...
        const mongodVersion = instanceState.cachedConnectionInfo()?.buildInfo?.version;
        ...
        await Promise.race([
            new Promise((resolve) => this.bus.once('mongosh:eval-interrupted', resolve)),
    ...
    writer(result, extraFormatOptions) {
        ...
        return this.formatShellResult(this.rawValueToShellResult.get(result) ?? {
            type: null,
            printable: result,
        }, extraFormatOptions);
    }
    ...
    async onPrompt(question, type) {
        ...
        if (type === 'password') {
            ...
            this.output.write('\n');
            return { Y: 'yes', N: 'no' }[result.toUpperCase()] ?? '';
        }
    ...
    async setConfig(key, value) {
        ...
        if (result === 'success') {
            if (key === 'historyLength' && this._runtimeState) {
                const historyManager = this.runtimeState().repl.historyManager;
                if (historyManager) {
                    const historyManagerSymbols = Object.getOwnPropertySymbols(historyManager);
                    const kSize = historyManagerSymbols.find((symbol) => String(symbol).includes('(_kSize)'));
                    if (kSize) {
...
function isErrorLike(value) {
    try {
        ...
        throw new errors_1.MongoshInternalError(err?.message || String(err));
    }
}
